...
describe("S14 grounded provider composition", () => {
  it("keeps the exact approved stateless Gemini profile with a versioned product prompt", async () => {
    ...
    expect(
      await createGroundedAnswerAIProvider(config, { fetch: fetchMock }).decide(input),
    ).toMatchObject({ kind: "completed", model: "gemini-3.8-flash" });
    ...
    expect(sent).toMatchObject({
      systemInstruction: { parts: [{ text: GROUNDED_ANSWER_INSTRUCTIONS }] },
      generationConfig: {
    ...
    expect(request[1].body).not.toMatch(/"grounding"|"subject"|published_by_user_id|customer_id/u);
