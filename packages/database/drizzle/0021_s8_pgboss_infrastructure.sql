-- pg-boss package version: 12.31.0
-- pg-boss schema version: 41
-- getConstructionPlans("pgboss") SHA-256: 47cb669984703400f9d39066d3c3ce649d5ffab050967125295397c29da592ef
DO $role$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_catalog.pg_roles
    WHERE rolname = 'lead_agent_queue_runtime'
  ) THEN
    CREATE ROLE lead_agent_queue_runtime LOGIN;
  END IF;
END
$role$;

ALTER ROLE lead_agent_queue_runtime
  NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOREPLICATION NOBYPASSRLS;

ALTER ROLE lead_agent_queue_runtime SET search_path = pg_catalog;
...
REVOKE lead_agent_queue_runtime
  FROM lead_agent_runtime, lead_agent_ingress, lead_agent_auth,
  lead_agent_inbound_route_definer, lead_agent_identity_definer,
  lead_agent_membership_definer;

REVOKE ALL PRIVILEGES ON SCHEMA public, app
  FROM lead_agent_queue_runtime;
...
BEGIN
  EXECUTE pg_catalog.format(
    'GRANT CONNECT ON DATABASE %I TO lead_agent_queue_runtime',
    current_database()
  );
END
$database_grant$;
...
CREATE INDEX IF NOT EXISTS job_dep_parent_idx ON pgboss.job_dependency (parent_name, parent_id);
  ...
  ;
    ...
    LANGUAGE plpgsql;
  ;
INSERT INTO pgboss.version(version) VALUES ('41');
--> statement-breakpoint
SELECT pgboss.create_queue('inbound', '{"policy":"standard"}'::jsonb);
SELECT pgboss.create_queue('ai', '{"policy":"standard"}'::jsonb);
SELECT pgboss.create_queue('outbound_message', '{"policy":"standard"}'::jsonb);
SELECT pgboss.create_queue('staff_notification', '{"policy":"standard"}'::jsonb);
SELECT pgboss.create_queue('analytics', '{"policy":"standard"}'::jsonb);
SELECT pgboss.create_queue('maintenance', '{"policy":"standard"}'::jsonb);

REVOKE ALL PRIVILEGES ON SCHEMA pgboss
  FROM PUBLIC, lead_agent_runtime, lead_agent_ingress, lead_agent_auth,
  lead_agent_inbound_route_definer, lead_agent_identity_definer,
  lead_agent_membership_definer, lead_agent_queue_runtime;
...
REVOKE ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA pgboss
  FROM PUBLIC, lead_agent_runtime, lead_agent_ingress, lead_agent_auth,
  lead_agent_inbound_route_definer, lead_agent_identity_definer,
  lead_agent_membership_definer, lead_agent_queue_runtime;
...
GRANT SELECT ON TABLE pgboss.version, pgboss.queue
  TO lead_agent_queue_runtime;

GRANT SELECT, INSERT, UPDATE, DELETE
  ON TABLE pgboss.job, pgboss.job_common, pgboss.job_dependency
  TO lead_agent_queue_runtime;
