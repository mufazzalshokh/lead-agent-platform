...
**Decision:** Use PostgreSQL 17 for authoritative domain data,
idempotency/inbox/outbox, pg-boss jobs, and initial analytics facts. Local,
test, staging, and production use the latest supported PostgreSQL 17.x minor;
minor security/bugfix upgrades are allowed, while a future major upgrade
requires an explicit reviewed migration/compatibility decision. Use UTC
timestamps, exact numeric/integer minor units, explicit foreign
keys/checks/unique constraints/indexes, and managed backups/PITR in production.
...
**Decision:** V1 states are `requested -> staff_accepted -> awaiting_customer_confirmation -> confirmed`. Staff may reject/cancel; expiration is deterministic. `staff_accepted` moves to `awaiting_customer_confirmation` only after confirmation intent is durably queued. Frozen confirmed V1 evidence sources are `customer_session`, `telegram`, or an authorized `staff_attested_external` record with method `phone` or `in_person`; attestation is fully audited. Owner-approved S18 adds trusted Instagram Business DM evidence through confirmed V2 only, preserving V1 unchanged. Every confirmation must match both the expected aggregate version and current `offer_version` and use an explicit clock instant in `[issued_at, expires_at)`; equality with `expires_at` is expired. The S18 P0 window is `min(issued_at + 24 hours, accepted_start_at)` and cannot be renewed by retries/provider windows. AI may create a request but cannot authoritatively confirm or write a calendar.
...
| **S11 Messaging integrations:** Telegram Business DM + Instagram Business DM | S2,S8-S10 | S11.A Telegram Business bot/webhook/worker; S11.B Instagram; S11.C combined acceptance | Existing business DM customer experience; secure connection binding, provider rights, no terminal reopen, canonical normalize/send/status/errors; Telegram route mutation is EXECUTE-only via 0026 | onboarding forgery/CAS/collision, duplicate/reorder/thread/terminal/ambiguous send, two-tenant provider tests | Both mandatory channels pass combined hostile PostgreSQL + GitHub Actions acceptance; see `s11-telegram-business.md` |
...
| **S16 Appointment request workflow:** staff-controlled booking states | S3,S5,S8,S15 | domain/application bookings | Owner-approved fixed `s16_appointment_submission.v1` validates preference and creates `requested`; `business_policy_id` remains Qualification V1 provenance, not configurable booking authority; acceptance/rejection operations deferred to S17, confirmation to S18; no calendar write | requested creation/race/tenant/timezone/idempotency/atomicity suite | Nothing except customer action/attestation can reach `confirmed`; no configurable booking policy without separate freeze and historical compatibility decision |
...
| **S18 Customer confirmation delivery:** close booking safely | S8,S10-S11,S16-S17 | channel renderers, worker handlers, booking commands | Acceptance durably queues confirmation; `customer_session`, `telegram`, audited authorized `staff_attested_external` (`phone` or `in_person`), or trusted Instagram confirmed V2 evidence; fixed P0 expiry `min(issued_at + 24h, accepted_start_at)`, no renewal | crash/ambiguous delivery/duplicate/late/cancel/source/method/attestation and V1/V2 compatibility E2E | Confirmed implies allowed customer evidence and transition history; frozen V1 unchanged |
...
S20's implementation contract is frozen in
[`24-s20-analytics-observability-cost.md`](24-s20-analytics-observability-cost.md).
Normal tenant analytics exposes business value and plan/usage consumption, not
raw provider COGS or platform margin. Tenant economics is operator-internal;
unknown subscription revenue or cost allocation remains unavailable rather than
zero. S20 does not introduce billing or claim a production SLA.
...
| Telegram terminal Conversation policy — FROZEN | Product + integrations | No automatic reopen/window; next distinct customer message after resolved/closed creates a new cycle within the accepted grouping identity | Approved S11.A |
| Telegram bot ownership/customer experience — FROZEN | Product + integrations | One platform Business Bot behind existing business-account DMs; no per-tenant bot token or MTProto automation; secure nonce → owner → Business Connection binding | Approved S11.A |
| What further customer-confirmation UX/legal, configurable expiry, staff-review/reminder, reschedule/new-offer rules apply per supported channel? | Product + privacy/legal + domain | S18 resolves only fixed P0 customer expiry, existing cancel-only decline and additive Instagram evidence; configurable policies/UX/legal requirements remain tracked | Before the corresponding future capability |
...
| Widget loader/iframe CSP, framing and host-page integration — FROZEN | Product + frontend/security | Cross-origin sandboxed platform iframe; <=60-second opaque single-use host-bound exchange; bearer stays inside iframe; exact-origin CSP/postMessage; third-party-cookie independent | Approved S19; browser evidence remains in S19 acceptance |
| Staff sensitive/health-adjacent visibility, exports, and support access — FROZEN | Privacy + security + product | Backend-authorized projections only; authorized customer text may be displayed without clinical interpretation; exports and support access remain deferred | Approved S19; medical wording/readiness remains S21b |
