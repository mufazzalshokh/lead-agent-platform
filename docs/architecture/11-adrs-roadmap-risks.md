...
**Decision:** Maintain canonical JSON Schemas in `packages/contracts` and derive/generate TypeScript/OpenAPI representations where tooling supports it. Every external input/output and durable event has a schema/version and compatibility test.
...
10. **No vector store versus multilingual grounding:** V1 retrieves bounded structured same-tenant facts through PostgreSQL and tests recall; measured failure triggers a new evaluated/security-reviewed ADR, not speculative infrastructure.
